...
- **랜덤 학습**: 홈 오른쪽 위 🔀 → 지문을 고르고(그룹 제목 = 그룹 전체, 왼쪽 아래 ✓✓ = 전체) ▶. 고른 지문의 문제를 끝없이 섞어서 냄. 한 바퀴 안에서는 겹치지 않고, 다 돌면 새 순서로 다시 시작
...
- 처음부터 모든 항목이 섞여서 놓여 있고, 끌어서 순서만 바꿈. 긴 항목(문단)은 왼쪽 손잡이를, 짧은 항목(단어 칩)은 칩 자체를 잡고 끌면 됨. 화면 위·아래 끝으로 가져가면 페이지가 따라서 스크롤됨
