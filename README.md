...
- **랜덤 학습**: 홈 오른쪽 위 🔀 → 지문을 고르고(그룹 제목 = 그룹 전체, 왼쪽 아래 ✓✓ = 전체), 위쪽 유형 칩으로 낼 유형만 켜 두고(문단배열·문장배열 등), **안 푼 문제** 칩을 켜면 풀이 기록이 없는 문제만 냄 ▶. 고른 지문의 문제를 끝없이 섞어서 냄. 한 바퀴 안에서는 겹치지 않고, 다 돌면 새 순서로 다시 시작(안 푼 문제 모드에서는 그새 푼 문제를 뺌. 풀지 않고 넘긴 문제는 다음 바퀴에 다시 나오고, 전부 풀면 완료 화면이 나와 ↻로 푼 문제까지 포함해 계속할 수 있음)
